...
public static class DeadFile
{
    public static readonly string Dir = Path.Combine(TShock.SavePath, "怪物加速", "死亡事件");
    ...
    public static List<TimerData> Load(string name)
    {
        ...
        if (!File.Exists(path))
        {
            TShock.Log.ConsoleError($"{LogName} 死亡事件文件不存在: {name}");
        ...
        {
            TShock.Log.ConsoleError($"{LogName} 读取死亡事件失败 {name}: {ex.Message}");
            return new List<TimerData>();
        }
    }
