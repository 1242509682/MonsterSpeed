...
{
    #region 设置指示物
    public static bool SetMarker(NpcState state, string mName, string[] ops, ref UnifiedRandom rand, NPC src = null!)
    {
        ...
        try
        {
            ...
            foreach (string op in ops)
            {
                if (!string.IsNullOrEmpty(op))
                    final = ApplyOp(curr, op, state, ref rand);
            }
    ...
    #region 应用操作
    private static int ApplyOp(int curr, string op, NpcState st, ref UnifiedRandom rnd)
    {
    ...
    #region 解析数学
    private static int ParseMath(int curr, string expr, NpcState st)
    {
        if (string.IsNullOrEmpty(expr)) return curr;
    ...
    #region 解析数值
    private static int ParseVal(string vStr, NpcState st)
    {
        ...
        try
        {
            if (vStr.StartsWith("ref:") || vStr.StartsWith("using:") || vStr.StartsWith("use:"))
            {
                ...
                return (int)(GetMarker(st, refName) * factor);
            }
    ...
    #region 检查条件
    public static bool CheckMarkers(NpcState st, Dictionary<string, string[]> conds, NPC src = null!)
    {
        ...
        {
            if (!CheckSingle(st, cond.Key, cond.Value, src))
                return false;
        }
        return true;
    }
    ...
    #region 检查单个
    private static bool CheckSingle(NpcState st, string mName, string[] exprs, NPC src)
    {
        ...
        try
        {
            ...
            {
                if (!string.IsNullOrEmpty(expr) && !EvalCond(curr, expr, st))
                    return false;
            }
            return true;
        }
    ...
    #region 评估条件
    private static bool EvalCond(int curr, string expr, NpcState st)
    {
    ...
    #region 批量设置
    public static int SetMarkers(NpcState st, Dictionary<string, string[]> ops, ref UnifiedRandom rnd, NPC src = null!)
    {
        ...
        {
            if (SetMarker(st, op.Key, op.Value, ref rnd, src))
                count++;
        }
        return count;
    }
    ...
    #region 设置怪物指示物
    public static int SetMstMarkers(List<MstMarkerMod> mods, NPC src, ref UnifiedRandom rnd)
    {
        if (mods == null || src == null) return 0;

        var sState = StateUtil.GetState(src);
        ...
        foreach (var mod in mods)
        {
            ...
            {
                if (!ValidTarget(i, mod, src)) continue;

                var tState = StateUtil.GetState(Main.npc[i]);
                if (tState == null) continue;

                int modded = SetMarkers(tState, mod.MarkerMods, ref rnd, src);
                total += modded;
            }
        }
    ...
    #region 验证目标
    private static bool ValidTarget(int idx, MstMarkerMod mod, NPC src)
    {
        if (idx < 0 || idx >= Main.maxNPCs) return false;

        var tNpc = Main.npc[idx];

        if (!PxUtil.IsValidMst(tNpc, src)) return false;
        if (mod.MstID != 0 && tNpc.netID != mod.MstID) return false;
        if (mod.Range > 0 && !PxUtil.InRangeTiles(src.Center, tNpc.Center, mod.Range)) return false;

        var tState = StateUtil.GetState(tNpc);
        if (tState == null) return false;

        if (mod.MarkerConds != null && mod.MarkerConds.Count > 0 &&
            !CheckMarkers(tState, mod.MarkerConds, src))
            return false;

        return string.IsNullOrEmpty(mod.Flag) ||
               (MonsterSpeed.Config?.Dict?.ContainsKey(tNpc.FullName) == true &&
                MonsterSpeed.Config.Dict[tNpc.FullName].Flag == mod.Flag);
    }
    #endregion
}