...
```
v1.3.8.4
重构大部分代码,优化条件检查性能与重复代码（增加注释）
加入了《碰撞事件》调用方式与《弹幕命中玩家》、《死亡事件》相同
（有考虑过把这3个事件作为条件检查使用，但独立运行能增加怪物技能多样性）
以上3类事件均支持循环执行，人话：可写多个文件名

v1.3.8.3
重构部分代码优化性能
修复了服务器无人时,再次生成出来的怪物血量很低BUG
简化更新弹幕中的《自动跟踪》：
1.移除了弹幕躲避功能，移除“追踪类型”
2.加入了“最近玩家”（只遍历攻击过npc的玩家选出距离最近）
死亡事件与弹幕命中事件不再需要独立文件夹，直接从《时间事件》文件夹读取文件名即可。
弹幕命中与死亡事件支持条件检查
“回血比例”支持0.1（表示每次回复千分之一血量）

v1.3.8.2
加入了“统一”：难度人数、难度乘数、动态血量、回血比例、回血间隔
用于修改不在《怪物数据表》与《怪物ID表》的其他怪物：
统一难度人数：推荐2-4（影响单人基础数值，拥有2到4倍数值）
统一难度乘数：不建议修改（最终生命上限、碰撞伤害、防御都会乘于它）
统一动态血量：随着玩家人数增加减少,自动修改所有小怪血量(0为关闭，0.1为每人增减10%血)
统一回血间隔：单位为秒，意味着多少秒回一次血，默认为10
统一回血比例：1 为每间隔10秒回1%的血量，0为关闭回血。
注意：
《怪物数据表》相同参数优先级 > "统一"字段的配置项
"统一"有自己的《排除怪物表》,该表不影响《怪物数据表》内的“怪物ID”
本插件默认排除:城镇NPC、雕像怪、假人傀儡、捕捉类小动物、友好NPC
(任何表内输入以上相关ID都不会受到影响)
新增配置项《报血间隔》 0为关闭 > 0为开启（播报血量变化冷却,单位毫秒）

v1.3.8.1
加入了友好弹幕也能伤害玩家判断
加入了弹幕命中玩家事件(只需要写时间事件文件名即可,读取的文件夹:怪物加速 - 弹幕命中事件)

v1.3.8
重构部分重复性代码
玩家被敌对弹幕击杀也能计入击杀数
加入了死亡事件(只需要写时间事件文件名即可,读取的文件夹:怪物加速 - 死亡事件)
加入了难度人数调整npc强度用：难度人数为0使用原版逻辑，>0强制当作这么多玩家
加入了难度乘数：最终生命上限和伤害值 都会乘这个值
加入了动态血量：服务器每增加1人，boss自动调整血量1倍血量，0为关闭（支持小数点百分比）
