...
public class ProjData
{
    ...
    public float DecayMultiplier = 1.0f; // 默认值1.0表示不额外调整
    ...
    public int Lift = 120;
    [JsonProperty("以玩家为中心", Order = 14)]
    public bool TarCenter = false;
    [JsonProperty("更新间隔", Order = 15)]
    public double UpdateTime = 500f;
    [JsonProperty("更新弹幕", Order = 16)]
...
public class UpdateProjData
{
    ...
    [JsonProperty("追踪模式", Order = 29)]
    public bool Homing = false;
    [JsonProperty("追踪目标(0怪物/1玩家/2弹幕)", Order = 30)]
    public int Tar = 0;
    [JsonProperty("预测秒数", Order = 31)]
    public float PredictTime = 0f;
    [JsonProperty("追踪强度", Order = 32)]
    public float HomingStrength = 1.0f;
    [JsonProperty("追踪角度", Order = 33)]
...
internal class MyProjectile
{
    ...
    public static void SpawnProjectile(List<ProjData> SpawnProj, NPC npc)
    {
        ...
        Terraria.DataStructures.NPCAimedTarget tar = npc.GetTargetData(true); // 获取玩家与怪物的距离和相对位置向量
        float range = Vector2.Distance(tar.Center, npc.Center);
        Vector2 dict = tar.Center - npc.Center; // 目标到NPC的方向向量

        // 获取NPC的状态
        SpawnProjState? state = GetState(npc);
        ...
        else
        {
            ...
            // 计算衰减值
            float oldDecay = proj.decay; // 基础衰减值
            float DecayMultiplier = oldDecay * proj.DecayMultiplier; // 实际衰减值
            float decay = 0;
            //随着弹幕数量的增加而减慢
            if (proj.decayForStack)
            {
                decay = 1.0f - state.SendStack[state.Index] / (float)proj.stack * DecayMultiplier;
            }
            else if (DecayMultiplier != 0)
            {
        ...
        // 修复：将更新调用移到外面，确保每次SpawnProjectile被调用时都会检查更新
        UpdateAllProjectiles(npc, plr, SpawnProj);
    }

    // 新增：单独的处理所有弹幕更新的方法
    private static void UpdateAllProjectiles(NPC npc, Player plr, List<ProjData> SpawnProj)
    {
        for (var i = 0; i < UpdateState.Length; i++)
        {
            ...
            UpdateProj upState = UpdateState[i];

            // 检查这个弹幕是否属于当前NPC
            if (upState.whoAmI != npc.whoAmI) continue;
            ...
            if (state.Index >= SpawnProj.Count) continue;

            ProjData proj = SpawnProj[state.Index];
            if (proj.UpdateProj == null || proj.UpdateProj.Count == 0) continue;

            // 检查更新间隔
            if (UpdateTimer.ContainsKey(index) &&
                (DateTime.UtcNow - UpdateTimer[index]).TotalMilliseconds >= proj.UpdateTime)
            {
                UpdateProjectile(proj, proj.UpdateProj, npc, plr, upState.Type, proj.stack, state);
                UpdateTimer[index] = DateTime.UtcNow;
            }
        }
    }
    #endregion
    ...
    #region 更新弹幕方法
    public static UpdateProj[] UpdateState { get; set; } = new UpdateProj[Main.maxProjectiles];
    private static void UpdateProjectile(ProjData proj, List<UpdateProjData> Update, NPC npc, Player plr, int type, int stack, SpawnProjState state)
    {
        ...
        for (var i = 0; i < UpdateState.Length; i++)
        {
            ...
            if (NewProj == null || !NewProj.active || NewProj.type != upState.NewType || NewProj.owner != Main.myPlayer)
            {
                UpdateState[i] = null!;
                continue;
            }
            ...
            // 检查是否还有更多更新阶段
            if (upState.UpdateIndex >= Update.Count)
            {
                continue;
            }
            ...
            {
                if ((DateTime.UtcNow - upState.StageStartTime).TotalMilliseconds < up.StageInterval)
                {
                    continue;
                }
            }
            ...
            // 处理弹幕类型变更逻辑
            if (up.NewType != 0) // 只有不为0时才处理类型变更
            {
                int newType;
                if (up.NewType == -1)
                {
                    // -1：恢复原始弹幕ID
                    newType = upState.Type;
                }
                else if (up.NewType > 0)
                {
                    // 大于0：替换为新弹幕ID
                    newType = up.NewType;
                }
                ...
                {
                    // 其他情况保持当前类型
                    newType = upState.NewType;
                }

                // 只有当类型确实发生变化时才更新
                if (newType != upState.NewType && newType > 0)
                {
                    // 更新弹幕类型
            ...
            // 计算衰减值
            float OldDecay = up.decay;
            float DecayMultiplier = OldDecay * up.DecayMultiplier;
            float decay = 0;

            if (up.decayForStack)
            {
                decay = 1.0f - state.SendStack[state.Index] / (float)stack * DecayMultiplier;
            }
            else if (DecayMultiplier != 0)
            {
                decay = DecayMultiplier;
            ...
            float speed2 = (float)(velocity != 0 ? velocity : NewProj.velocity.Length());

            // 计算速度向量
            Vector2 vel = NewProj.velocity.SafeNormalize(Vector2.Zero) * speed2;

            // 获取攻击目标
            Entity tar = GetTarget(up.Tar, npc, plr, NewProj);

            // 应用追踪模式
            vel = HomingMode(vel, NewProj, tar, up, upState, ref UpList);

            // 计算角度偏移
            double angle = up.Angle * Math.PI / 180;
            double radian = angle != 0 ? angle : 0;
            ...
            // 重置阶段开始时间
            upState.StageStartTime = DateTime.UtcNow;
        }
    ...
    #region 追踪模式方法
    private static Vector2 HomingMode(Vector2 vel, Projectile proj, Entity tar, UpdateProjData up, UpdateProj state, ref List<int> upList)
    {
        Vector2 result = vel;

        // 追踪模式 - 只有当目标玩家有效时才追踪
        if (up.Homing && tar != null && tar.active)
        {
            ...
            if (up.PredictTime > 0)
            {
                pos += tar.velocity * up.PredictTime * 60f; // 预测目标位置
            }
    ...
    #region 目标类型选择方法
    public static Entity GetTarget(int tarType, NPC npc, Player plr, Projectile proj)
    {
        switch (tarType)
        {
            case 0: // NPC
    ...
    public static void ClearUpState(NPC npc)
    {
        var Remove = UpdateTimer.Keys
            .Where(key =>
            {
                return UpdateState[key] != null && UpdateState[key].whoAmI == npc.whoAmI;
            })
            .ToList();
